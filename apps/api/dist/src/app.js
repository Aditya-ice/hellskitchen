...
import { fallbackDishContext, ingredients, menuItems, recommendDishes, recommendTables, restaurant, staff, } from "@hellskitchen/shared";
import { enforceRateLimit, getClientIp, issueSessionToken, requireAuth, } from "./auth";
...
export function createServerApp() {
    ...
    api.post("/guests/:guestId/seat", async (c) => {
        ...
        if (updated === stateBefore) {
            return c.json({ error: "Table cannot seat this guest. Check table status, capacity, or accessibility." }, 400);
        }
        return c.json(updated);
    });
    ...
    api.post("/orders/:guestId/send", (c) => {
        ...
        if (updated === stateBefore) {
            return c.json({ error: "Cannot send order. Make sure order is in draft status and contains items." }, 400);
        }
        return c.json(updated);
    });
    api.get("/integrations/elevenlabs/token", async (c) => {
        ...
        try {
            ...
            return c.json({ error: "Voice transcription is temporarily unavailable." }, 502);
        }
    });
