...
describe("API server routes & auth", () => {
    ...
    it("allows state retrieval and mutations with a valid session token", async () => {
        ...
        expect(afterCheckIn.guests.find((g) => g.id === "guest-jordan").status).toBe("waiting");
        // Recommendations
        ...
        expect(afterSeat.tables.find((t) => t.id === "t2").seatedGuestId).toBe("guest-maya");
        // Add order item
        ...
        expect(afterSend.orders.find((o) => o.guestId === "guest-maya").status).toBe("sent");
        // Reset
